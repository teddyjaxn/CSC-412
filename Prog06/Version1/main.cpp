...
void *threadFunc(void *arg)
{
	...
	while (!fullStop)
	{
		...
		if (printGrid == false)
		{
			...
			// for loop creating the array of pthread objects, assigning
			// them relevant properties/assignmnets, then creating them.
			for (unsigned int i = 0; i < maxThreads; i++)
			{
			...
			for (unsigned int i = 0; i < maxThreads; i++)
			{
				...
				if (pthread_join(TI[i].threadID, NULL) != 0)
				{
					cout << "bad Kill" << endl;
				}
				numLiveThreads -= 1;
...
void *oneGeneration(void *arg)
{
	...
	//cout << localData->threadIndex << " is starting, first row is " << localData->globalContextStartRow
	//<<	" and will run for " << localData->assignedRows << " rows" << endl;
